---
title: Web Tasarım Freelance — Modüler Fiyatlandırma Sistemi
created: 2026-08-27
modified: 2026-08-28
type: project
status: active
tags: [freelance, web-tasarim, fiyatlandirma, modüler, ekip]
---

# Web Tasarım Freelance — Modüler Fiyatlandırma Sistemi

**Ekip:** Mehmet Akif Akkaya (satış, müşteri ilişkileri) + Dündar Dinç (tasarım, teknik, proje yönetimi)

**Felsefe:** Müşteri neyi isterse o kadarını öde — tek index sayfası mı, çok sayfası mı, reklam mı, e-commerce mi, hepsi ayrı modüler.

---

## 1. BASE SITE (Anahtar Teslim - Tek Index Sayfası)

Domain + DNS + canlıya alma dahil. Hızlı iş, kısa hazırlanma.

| Paket | Tasarım Tipi | Hazırlanma | Fiyat |
|---|---|---|---|
| **Basic** | Standart tasarım, responsive, interaktif | 2-3 gün | **2.000₺** |
| **Standart** | Gelişmiş tasarım, daha fazla özellik | 4-7 gün | **5.000₺** |
| **Premium** | Video-scroll animasyonlu, profesyonel | 10-14 gün | **12.000₺** |

---

## 2. SAYFALARı EKLEME (Sayfa Başı - Opsiyonel)

Müşteri "Ürünler sayfası", "Hakkımızda", "İletişim" vb. isterse, her sayfaya ayrı ücret.

| Paket | Karmaşıklık | Fiyat/Sayfa |
|---|---|---|
| **Basic** | Standart sayfa (basit layout) | 500₺ |
| **Standart** | Gelişmiş sayfa (filtreleme, carousel vb) | 1.000₺ |
| **Premium** | Kompleks sayfa (animasyon, interaktif) | 2.000₺ |

---

## 3. AYLYK ABONELİK HIZMETLERI

Müşteri aylık destek/bakım isterse seçer. Kalite seviyesine göre fiyat değişir.

### Bakım Paketi (Content Update + Monitoring)

| Paket | Detay | Fiyat/ay |
|---|---|---|
| **Basic** | Text/görsel update, monitoring, temel destek | 300₺ |
| **Standart** | Regular updates, SEO temel, monitoring, destek | 600₺ |
| **Premium** | Kapsamlı updates, 24/7 monitoring, advanced SEO | 1.000₺ |

### Premium Support (Konsultasyon + Strategy + Raporlar)

| Paket | Detay | Fiyat/ay |
|---|---|---|
| **Basic** | Haftalık raporlar, email destek | 500₺ |
| **Standart** | Haftalık raporlar, strategy danışmanlığı, priority destek | 1.000₺ |
| **Premium** | Haftalık detaylı raporlar, monthly strategy call, 24/7 priority destek | 1.500₺ |

### Reklam Yönetimi (Social Media + Google Ads)

| Paket | Detay | Fiyat/ay |
|---|---|---|
| **Basic** | Google Ads temel kurulum ve yönetimi | 800₺ |
| **Standart** | Google Ads + Instagram Ads, haftalık optimizasyon | 1.500₺ |
| **Premium** | Google + Instagram + LinkedIn, advanced strategy, daily monitoring | 2.500₺ |

---

## 4. E-COMMERCE (Online Satış Sistemi - Opsiyonel)

Müşteri "müşteriler online satın alabilsin" isterse backend ve payment gateway eklenir.

| Paket | Özellik | Fiyat (Anahtar Teslim) | Aylık (Bakım + Payment Processing) |
|---|---|---|---|
| **Basic E-Commerce** | Basit ürün sayfası + checkout | +3.000₺ | 500₺ + %2,5 commission |
| **Standart E-Commerce** | Ürün kategorileri, sepet, inventory tracking | +6.000₺ | 800₺ + %2 commission |
| **Premium E-Commerce** | Gelişmiş inventory, müşteri hesabı, CRM entegrasyonu | +10.000₺ | 1.200₺ + %1,5 commission |

---

## 5. ÖRNEK SİPARİŞ HESAPLAMALARI

### Örnek 1: Kuaför (Basit)
- Base Site (Basic): 2.000₺
- Sayfa Ekleme (2 sayfa - Hizmetler, İletişim): 500₺ × 2 = 1.000₺
- **Toplam:** 3.000₺ (anahtar teslim)
- Aylık (Bakım + Reklam): 300₺ + 800₺ = 1.100₺/ay

### Örnek 2: Kafe (Orta)
- Base Site (Standart): 5.000₺
- Sayfa Ekleme (3 sayfa - Menü, Galeri, İletişim): 1.000₺ × 3 = 3.000₺
- Reklam Yönetimi eklenirse: +1.500₺/ay
- **Toplam:** 8.000₺ (anahtar teslim)
- Aylık: 600₺ (Bakım) + 1.500₺ (Reklam) = 2.100₺/ay

### Örnek 3: Çevrimiçi Satış (Kompleks)
- Base Site (Premium): 12.000₺
- Sayfa Ekleme (4 sayfa): 2.000₺ × 4 = 8.000₺
- E-Commerce (Premium): +10.000₺
- **Toplam:** 30.000₺ (anahtar teslim)
- Aylık: 1.000₺ (Bakım) + 1.200₺ (E-Commerce) + 2.500₺ (Reklam) = 4.700₺/ay + satış komisyonu

---

## İŞ AKIŞI

1. **Müşteri Kazanım (Akif):** Esnafa git, ihtiyaçlarını dinle, hangi modülü istediğini anla
2. **Teknik Plan (Dündar):** Seçilen modülleri hesapla, teklif sun
3. **Tasarım & Geliştirme (Dündar):** Modullar doğrultusunda site yap
4. **Aylık Destek (Dündar):** Seçilen subscription paketlerine göre çalış

---

## AÇIKLAMALAR

- Her şey **modüler**: müşteri sadece index isterse 2k, 10 sayfa + e-commerce isterse 30k
- **Hazırlanma süresi** sadece seçilen paketler için geçerli (Base + Sayfalar)
- **Aylık abonelikler** bağımsız, müşteri dilerse hiçbirini seçmeyebilir, dilerse tümünü seçebilir
- **E-commerce commission:** Payment gateway (Stripe, PayPal vb) ücretinden %1-2,5 kesintisi Akif ve Dündar arasında bölünür

## İş Akışı

### Müşteri Kazanım (Akif)
1. Hedef esnaf belirleme (kuaför, kafe, klinik vb.)
2. Yüz yüze prezentasyon (tablet demo + paket seçenekleri)
3. Sipariş alınca Dündar'a devretme

### Tasarım & Teknik (Dündar)
1. Müşteri briefi al
2. Şablon seç / özelleştir
3. Tasarım tamamla
4. Domain satın al (müşteri adına)
5. Canlıya al ve test et
6. Teslim

### Aylık Destek (Dündar)
- Content update, monitoring
- Reklam yönetimi (varsa)
- Performans raporları

---

## Hedef: İlk 2 Hafta

**Hafta 1-2:** Basic/Standart paketlerden 3-4 müşteri bulup kapanış yap → **15-30k₺ nakit**

Ardından Premium paketleri ve reklam yönetimini ölçeklendirme.

---

## Tasarım Şablonları (Mevcut + Geliştirilenler)

- [ ] Basic sektörel şablonlar (kafe, kuaför, klinik) — standart tasarım
- [ ] Standart şablonlar (daha gelişmiş UX, daha fazla özellik)
- [ ] Premium şablonlar (video-scroll, animasyonlu geçişler)

**Sıradaki Adım:** İlk hedef sektörü seç, o sektöre ait Basic şablonunu 48 saat içinde hazırlayalım.

Hangi sektörle başlayalım?
